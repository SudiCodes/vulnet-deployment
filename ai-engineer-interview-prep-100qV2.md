...
**Last verified against web sources:** 23 July 2026

---

## ⚠️ Corrections from the source check (read these — v1 of this doc had errors)

1. **DSA is much heavier than I first said.** Infosys' AI Engineer loop explicitly includes rigorous algorithmic rounds — graph cycle detection, dynamic programming, tries, heaps — often in an Online Assessment before you ever discuss RAG. I originally told you to "refresh DSA lightly." That was wrong. **See the new Section 10.**
2. **The EU AI Act timeline in Q99 was outdated.** The Digital Omnibus on AI changed the dates in mid-2026. Corrected in place.
3. **India's DPDP now has hard dates** (Rules notified Nov 2025, phased to May 2027). Corrected in place — this matters more for you than the EU Act, and most candidates won't know it.
4. **The field's vocabulary has shifted to "context engineering."** Added to Section 4.
5. **Red-teaming / adversarial testing is a named job requirement**, not a nice-to-have. Added to Section 9.
...
46b. **What is "context engineering" and how is it different from prompt engineering?** *(new framing; increasingly the way these questions get asked in 2026)*
    → Context engineering is designing everything the model sees on each inference call, of which the prompt is one input. Four pillars: **instructions** (system prompt, tool descriptions), **retrieval** (RAG, search), **memory** (short-term conversation state, long-term persistent store), and **tools** (the function-calling surface, increasingly standardized via MCP). The line to use: *the center of gravity moved from wording to wiring.* Practical techniques: find the minimum set of high-signal tokens; place critical information at the start or end, never the middle; use just-in-time retrieval instead of pre-loading; separate static context from dynamic context so the static part can be prompt-cached. Related term you should recognize: **agent harness** — the infrastructure coordinating tool execution, memory, and state across an agent's session.
...
96b. **How would you red-team a GenAI system before it goes to a client?** *(Deloitte India's Responsible AI roles list adversarial testing and reusable test toolkits as must-have skills — this is a named job requirement, not a bonus.)*
    → Build reusable adversarial test suites: jailbreak and roleplay attacks, direct and indirect prompt injection (poisoned documents in the index), PII extraction and training-data probing, tool-abuse attempts against agent permissions, bias probes via counterfactual pairs, and refusal-consistency checks. Automate it into CI with a pass threshold, version the attack set, and produce a documented risk assessment artifact — the deliverable to a client is the report, not just the fix.
...
99. **What compliance frameworks apply to an AI deployment for an Indian enterprise with EU customers?**
    → Frameworks: EU AI Act, GDPR, India's DPDP Act 2023 + DPDP Rules 2025, NIST AI RMF and ISO/IEC 42001 as voluntary management standards, plus sector rules (RBI, IRDAI, HIPAA). Naming ISO 42001 and NIST AI RMF unprompted is a strong Big 4 signal.

    **EU AI Act — the dates moved this year, and knowing that is the differentiator.** The Commission proposed a "Digital Omnibus on AI" in Nov 2025; the Council gave it final approval on 29 June 2026, deferring high-risk obligations for stand-alone Annex III systems to 2 December 2027 and for AI embedded in Annex I regulated products to 2 August 2028. Critically, the Article 50 transparency obligations were **not** deferred and still apply from 2 August 2026 — that's next month. Article 50(2) transparency for legacy systems, plus a set of new prohibited practices, kicks in 2 December 2026. The Omnibus also added a prohibition on AI systems generating non-consensual intimate imagery and CSAM. If you say "the EU delayed the AI Act," you'll be half right and a sharp interviewer will catch it — some obligations slipped 16 months, others didn't move at all.

    **DPDP — know this cold; you're an India-based candidate and most applicants won't.** MeitY notified the DPDP Rules in November 2025 with three staggered enforcement dates: 14 Nov 2025, 14 Nov 2026, and 14 May 2027. In practice: the Data Protection Board is already operational, Consent Manager registration opens around Nov 2026, and full compliance plus penalty enforcement lands May 2027, with penalties running up to ₹250 crore. Two provisions bite AI systems directly: verifiable parental consent for processing data of anyone under 18, and deletion of personal data once its purpose is fulfilled, with documented retention schedules. Translate that for the interviewer: **your RAG index and your agent's long-term memory store are both personal-data stores subject to erasure rights and retention limits.** Most teams haven't designed for that. Saying it out loud is a very strong signal in a Big 4 room.
...
## Section 10 — DSA for AI Engineers (the round I underweighted)

Infosys' AI Engineer loop starts with an Online Assessment focused on algorithmic problem-solving, and technical rounds carry genuinely hard graph and DP problems. The pattern across TCS/Infosys/Wipro is the same: **AI knowledge gets you shortlisted, DSA gets you rejected.** With 6 years of experience you'll be expected to code cleanly and state complexity before you write.

What's distinctive is that these firms increasingly wrap DSA in AI-flavored framing — same algorithms, new costume:

- **Cycle detection in a directed graph** framed as *"detect circular dependencies in an agentic workflow DAG"* → DFS with colors, or Kahn's algorithm.
- **Topological sort** framed as *"determine optimal execution order for dependent agent tasks."*
- **Top-k nearest neighbours** from an array of vector distances, **without using libraries** → size-k max-heap, O(n log k), with deterministic tie-breaking.
- **Edit distance / longest common subsequence** framed as *"deduplicate near-identical document chunks before embedding."*
- **Trie construction** framed as *"optimize prefix search across a massive unstructured text corpus."*
- **Maximum sum of non-adjacent nodes in a binary tree** → tree DP.
- Sliding window / two pointers on strings (chunking with overlap is literally a sliding window — expect it framed that way).

**Prep allocation:** graphs (BFS/DFS/topo sort/cycle detection) > dynamic programming (strings, sequence alignment) > heaps and top-k > tries > trees. Roughly 60–80 medium problems, not 400. Also refresh SQL — joins, window functions, CTEs — since these firms mix data rounds in.

**Do this out loud.** State approach → edge cases → data structure choice → time/space complexity → *then* code. Interviewers at these firms score the process explicitly.
...
**Week 1 — DSA + your story.** Section 10 daily (2 problems/day, timed, out loud) alongside Sections 1–4. Write out your project narrative; draw its architecture from memory in under 3 minutes. Recompute your project's actual cost and latency numbers — you will be asked for specifics.

**Week 2 — RAG + agents + serving.** Sections 5–8, DSA continuing daily. Build one small end-to-end thing (FastAPI + hybrid retrieval + reranker + eval harness) even if you've built similar before — recency of hands-on detail is what makes answers sound real.

**Week 3 — Design, governance, mocks.** Section 9 + system design out loud. 3 mock interviews. Refresh SQL. Re-read the regulatory dates the morning of any Big 4 round.

**Two extra hours, high leverage:** look up the AI platform brand of each firm you're interviewing with (Infosys has Topaz, and every one of these firms has an equivalent). Interviewers respond to candidates who reference their own platform by name.
